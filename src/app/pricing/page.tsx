            ...
            {plans.map((plan, index) => (
              ...
              >
                ...
                {plan.addOns.length > 0 && (
                  <div className="mb-8">
                    <h4 className="font-semibold text-slate-900 mb-3">Add-ons available:</h4>
                    <div className="space-y-2">
                      {plan.addOns.map((addOn, addOnIndex) => (
                        <div key={addOnIndex} className="flex justify-between items-center text-sm text-slate-600">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-2xl p-8 shadow-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <h3 className="text-lg font-semibold text-slate-900 mb-6">Your Current Business</h3>
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-900 mb-6">Your ROI with EZ Rate Sheet</h3>
                <div className="space-y-4">
                  <div className="flex justify-between">
                    <span className="text-slate-600">Current monthly income:</span>
                    <span className="font-semibold">${roiResults.currentMonthlyIncome.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Time saved per month:</span>
                    <span className="font-semibold text-emerald-600">{roiResults.timeSavingsHours.toFixed(1)} hours</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Additional deals possible:</span>
                    <span className="font-semibold text-emerald-600">+{roiResults.additionalDeals} deals</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Additional monthly income:</span>
                    <span className="font-semibold text-emerald-600">+${roiResults.additionalIncome.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Team plan cost:</span>
                    <span className="font-semibold text-red-600">-$149</span>
                  </div>
                  <div className="border-t pt-4">
                    <div className="flex justify-between text-lg">
                      <span className="font-semibold">Monthly ROI:</span>
                      <span className="font-bold text-emerald-600">+${roiResults.monthlyROI.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-xl mt-2">
                      <span className="font-semibold">Annual ROI:</span>
                      <span className="font-bold text-emerald-600">+${roiResults.annualROI.toLocaleString()}</span>
                    </div>
                    <div className="text-center mt-4 p-4 bg-success-50 rounded-lg">
                      <span className="text-2xl font-bold text-emerald-600">{roiResults.roiPercentage}% ROI</span>
